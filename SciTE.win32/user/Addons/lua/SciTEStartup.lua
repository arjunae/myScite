...
--print(props["Path"])
entry="AbbrevDlg	scite\win32\SciTEWinDlg.cxx	/^INT_PTR CALLBACK SciTEWin::AbbrevDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {$/;\"	f	class:SciTEWin	signature:(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)"
bla = entry:match("(%(.*%) )")
blu =entry:match("%\"%sf%s([%w:]+)") -- match " whitespace f whitespace, return Alphanum: (eg class:SciteWin)  
typ1M="%/^([%w_]+)%s"
typ2M="([%w_]+)%s"
classM="([%w]+).*"
funcM="(%(.*%))"
--typ1, typ2, parent, func= entry:match(typ1M..typ2M..classM..funcM)
--print(typ1,typ2,parent,func)