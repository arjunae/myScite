	...
	if (!importFiles.empty()) {
		for (int stackPos = 0; stackPos < static_cast<int>(importFiles.size()) && stackPos < importMax; stackPos++) {
			...
			if (importFiles[stackPos].IsSet()) {
				GUI::gui_string sFile = importFiles[stackPos].Name().AsInternal();
  				if (!(sFile.rfind(GUI_TEXT("theme"), 0) == 0 ||         
          			sFile.rfind(GUI_TEXT("tools"), 0) == 0 ||
          			sFile.rfind(GUI_TEXT("locale"), 0) == 0)) {
