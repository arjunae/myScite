		...
		for (int stackPos = 20; stackPos < static_cast<int>(importFiles.size()) && stackPos < importMax; stackPos++) {
			...
			if (importFiles[stackPos].IsSet()) {
				GUI::gui_string sEntry = importFiles[stackPos].Name().AsInternal();
        			auto pos = sEntry.find_last_of(GUI_TEXT("/\\"));
        			GUI::gui_string filename = (pos != GUI::gui_string::npos) ? sEntry.substr(pos + 1) : sEntry;
				GUI::gui_string sFile=localiser.Text("Open") + GUI_TEXT(" ") + filename;
				//  Depends on names to display properties sorted.
				if (sFile.find(GUI_TEXT("theme")) != GUI::gui_string::npos || sFile.find(GUI_TEXT("locale")) != GUI::gui_string::npos || sFile.find(GUI_TEXT("ools")) != GUI::gui_string::npos) {
