...
local lang = {}
	...
	lang["fr-classique"] = "Français classique"
	lang["es_ES"] = "Español"
	lang["it_IT"] = "Italiano"
...
-- Language settings
	...
	isoChars = "%a?-?" -- equal to ISO 8859-1 (Latin 1); redefine in the language section if necessary;
...
-- correct start position for UTF-8 chars starting with byte c2
	if string.sub(allText,sp,sp) == "?" then
		sp = sp + 1
	end
	return sp
end
