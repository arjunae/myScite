...
/// MSVC 2015 needs this for linking in statically.
#if _MSC_VER >= 1900
...
int do_run(lua_State* L)
{
	...
	size_t res = (size_t)ShellExecute (
		NULL,
		L"open",
		lpFile,
		lpParameters,
...
void window_aux_item(lua_State* L, bool at_index)
{
	...
	if(TListViewLua* lv = dynamic_cast<TListViewLua*>(w)) {
		...
		if (! lua_isnoneornil(L,next_arg+1)) {
			...
			data = (void*) ((intptr_t)luaL_ref(L,LUA_REGISTRYINDEX));
		}
...
static WNDPROC subclass(HWND hwnd, LONG_PTR newproc)
{
	WNDPROC old;
	if (::IsWindowUnicode(hwnd)) {
		old = (WNDPROC)SetWindowLongPtrW(hwnd, GWLP_WNDPROC, ((LONG_PTR)newproc));
		SetWindowLongPtrW(hwnd, GWLP_WNDPROC, newproc);
	} else {
		old= (WNDPROC)SetWindowLongPtrA(hwnd, GWLP_WNDPROC, ((ULONG_PTR)newproc));
...
static void subclass_scite_window ()
{
	...
	if (!subclassed) {  // to prevent a recursion
		old_scite_proc     = subclass(hSciTE,   (LONG_PTR)SciTEWndProc);
		old_content_proc   = subclass(hContent, (LONG_PTR)ContentWndProc);
		old_scintilla_proc = subclass(hCode,    (LONG_PTR)ScintillaWndProc);
		subclassed = true;
	}
}
...
/**
// @desc iterates through a given Path and returns its entries
// @param takes an optional bool to only return Directory entries.
// 23.06.17 | arjunea | More efficient and doesnt leak the handle anymore.
**/
...
static int do_files(lua_State *L)
{
	...
	intptr_t hFile = _wfindfirst(mask,&c_file);
	int idx = 1; // tables index

	if (hFile == -1L) {   /* no more entries => close directory */
		_findclose (hFile);
		lua_pushboolean(L,0);
		return 1; 
	} else {
	...
	do { // loops either through folders or files
		if ((((int)c_file.attrib & _A_SUBDIR) != 0) == look_for_dir) { 
			if (look_for_dir && (EQ(c_file.name,L".") || EQ(c_file.name,L".."))) { // skip them
			} else {
				lua_pushinteger(L,idx); 
				lua_pushstring(L,UTF8FromString(c_file.name)); //entry
				lua_settable(L,-3); // push current tables idx and entry
				idx++;
			}
		}
...
/**
// @description Worker metatable for do_dir_factory.
// @params Takes a valid path to iterate through.
// @returns A table containing the searchpath content and entries attributes.
// 25.07.17 | Arjunea | modified code from lfs.c. Has less latency penalties then gui.files.
**/
static int dir_iter (lua_State *L) {
	...
	if (d->hFile == 0L) { // errNoFirstFile
		if ((d->hFile = _findfirst (d->pattern, &c_file)) == -1L)				{
			...
			lua_pushinteger (L, c_file.attrib);
			return 2;
		} 
	/* next entry */	
	} else { 
...
/**
// @info customized Factory of directory iterators from lfs.c 
// @param LuaState
// @return a Lua Metatable function.
**/
static int do_dir_factory (lua_State *L) {
        ...
        lua_setmetatable (L, -2); // Set Metatable to fn dir_iter 
				d->closed = 0;
        d->hFile = 0L;
...
// Migrated functions from shell.dll

//------------------------------------------------------------------------------
// gui.inputbox(caption, prompt, default, check, modality, width)
//------------------------------------------------------------------------------
extern int show_inputbox( lua_State* );

//------------------------------------------------------------------------------
// gui.get_cliptext
//------------------------------------------------------------------------------
static int get_cliptext( lua_State* L )
{
	...
	lua_pushstring( L, UTF8FromString(clipText.c_str()) );
	return 1;
}
...
// gui.show_msgbox
//------------------------------------------------------------------------------
static int show_msgbox( lua_State* L )
{
...
/**
// @brief This Guy uses MS provided 
/ Unicode to widestring conversion functions  (see utf.h)
/ @param The codepage for the widestring
/ @return c widestring or 16bit multibyte string 
/ todo: move to utf.cpp
**/
...
static int internalConv( lua_State *L, bool toUTF8 )
{
	...
	if ( lua_isstring(L, 1) )
	{
		...
		//  default to encoding "AsciiCodePage"" or use codepage provided in lua param2
		ptrdiff_t codepage = luaL_optinteger( L, 2, CP_ACP );
		MB2W MBtoW( src, toUTF8? codepage:CP_UTF8 ); // c widestring to UTF8 (depends on CP set)
		if ( MBtoW.c_str() )
		{
			W2MB WtoMB( MBtoW.c_str(), toUTF8? CP_UTF8:codepage ); // UTF8 to c widestring
			if ( WtoMB.c_str() )
			{
				lua_pushstring( L, WtoMB.c_str() );
				success = true;
			}
		}
	}
...
//------------------------------------------------------------------------------
// gui.to_utf8 
// converts c widestring to UTF8 multibyte string
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// gui.from_utf8
// converts source multibyte string to a c widestring (ASCII encoded)
//------------------------------------------------------------------------------
...
static const struct luaL_Reg gui[] = {
	{"message",do_message}, // depreceated by gui.msgbox
	...
	{"run",do_run},
	{"chdir",do_chdir},
	{"files",do_files}, // depreceated by gui.dir
	{"set_panel",do_set_panel},
	{"pass_focus",do_pass_focus},
	{"dir",do_dir_factory},
	{"get_cliptext",get_cliptext},
	{"msgbox",show_msgbox},
	{"inputbox",show_inputbox},
	{"to_utf8",to_utf8},
	{"from_utf8",from_utf8},
	{NULL, NULL},
};
