...
void TToolbar::release()
{
     ...
     HANDLE hToolTip =(size_t *)((intptr_t)send_msg(TB_GETTOOLTIPS));
...
int TImageList::load_icons_from_module(const wchar_t* mod)
{
 ...
 int i = 1;
 while (
     (hIcon = (HICON)LoadImage(hInst, (const wchar_t*)((intptr_t)(i++)), IMAGE_ICON, cx, cy, LR_LOADMAP3DCOLORS))
     != NULL)
...
Handle TTreeView::add(Handle parent, const wchar_t* caption, int idx1, int idx2, bool has_children, void* data)
{
	...
	tvsi.item = item;
	return (Handle)(intptr_t)send_msg(TVM_INSERTITEM,0,(size_t)&tvsi);
}
