...
:  TEventWindow(L"",parent,0,true),
   m_form(parent),m_control(control),m_width(4 /*thick*/),m_min_size(30),m_line_visible(false),m_down(false)
{
...
{
   m_control->resize(rt); //seems to have issues on win64


   char buff[100];
   sprintf(buff,"%s right:%d left:%d bottom:%d top:%d","gui>resize",rt.right,rt.left,rt.bottom,rt.top );
   std::wstring wLog;
   wLog.resize(100, L'\0');
   mbstowcs(&wLog[0], buff, 100);
