...
{ return (void *)((intptr_t)send_msg(LB_GETITEMDATA,(int)i)); }
...
void subclass_control(TControl *ctrl)
{
 ...
 ctrl->m_wnd_proc = (void *)((intptr_t)GetWindowLong(hwnd,-4));
...
WNDFN FormWndProc (HWND hwnd, UINT msg, UINT wParam,LONG lParam)
{
 ...
 TEventWindow *ew = (TEventWindow *) ((intptr_t)GetWindowLong(hDlg,0));
 switch (msg) {
  case WM_KEYDOWN:
   if (wParam==VK_TAB) {
