...
#define LUA_UNSIGNED   unsigned LUAI_UACINT  // LUA5.3 
#else
...
*/		
		...
		luaL_newmetatable(L, crc32_reg_name);
		lua_pushstring(L, "__index");
		lua_pushvalue(L, -2);    
		lua_rawset(L, -3); 
		
		lua_pushcfunction (L, Lcrc_reset);
		lua_setfield(L, -2, "reset");
		lua_pushcfunction (L, Lcrc_update);
		lua_setfield(L, -2, "update");
		lua_pushcfunction (L, Lcrc_tonumber);
		lua_setfield(L, -2, "tonumber");
		lua_pushcfunction (L, Lcrc_tostring);
		lua_setfield(L, -2, "tostring");
		lua_pushcfunction (L, Lcrc_tohex);
