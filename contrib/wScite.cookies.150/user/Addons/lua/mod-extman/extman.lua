  ...
  if spawner then
    ...
    if  GTK then -- io.popen is dodgy; don't use it! 
      ...
      if Execute then -- scite_other was found!
        ...
        os.execute(cmd) -- this just returns the status code on win.
      end
     end
...
--scite_Command 'Open Command Prompt|OpenCmd|Ctrl+Alt+C'
--scite_Command 'Open Explorer|OpenExplorer|Shift+Ctrl+W'
...
dofile(props["SciteDefaultHome"]..'\\user\\Addons\\lua\\mod-extman\\eventmanager.lua')

-- Load SciTEStartup.lua
dofile('user\\Addons\\lua\\SciTEStartup.lua') 
