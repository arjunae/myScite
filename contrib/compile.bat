...
REM Otherwise, try to deduct make arch from gccs Pathname (searches for string mingw32)
...
REM Returns: DEST_PLAT Value: Either x86 or x64 
:find_platform
...
if exist ..\bin\SciTE.exe  (copy ..\bin\SciTE.exe %ReleaseDir% >NUL ) else (echo Error: unable to find build binaries & goto en )
if exist ..\bin\SciLexer.dll (copy ..\bin\SciLexer.dll %ReleaseDir% >NUL ) else (echo Error: unable to find build binaries & goto en) 
