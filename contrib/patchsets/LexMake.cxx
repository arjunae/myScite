 ...
 * @file LexMake.cxx
 * @author Neil Hodgson
 * @author Thorsten Kani(arjunae@nurfuerspam.de)
 ...
 * @brief 14.04.25 supported Styles (0-13): SCE_MAKE_DEFAULT, SCE_MAKE_COMMENT, SCE_MAKE_PREROC, SCE_MAKE_IDENTIFIER, SCE_MAKE_OPERATOR, SCE_MAKE_TARGET, SCE_MAKE_USER_VARIABLE, SCE_MAKE_FUNCTION, SCE_MAKE_DIRECTIVE, SCE_MAKE_IDEOL, SCE_MAKE_FLAGS, SCE_MAKE_EXTCMD, SCE_MAKE_STRING, SCE_MAKE_NUMBER
 * @brief todos
 * : Wrap within a Class. 
...
static unsigned int ColouriseMakeLine(
    ...
    while ( i < lengthLine ) {
        ...
        if(state==SCE_MAKE_DEFAULT && startMark==0 && IsNum(chCurr) && !(IsAlpha(chPrev))) {
            ColourHere(styler, currentPos-1, state);
...
static void ColouriseMakeDoc(Sci_PositionU startPos, Sci_Position length, int startStyle, WordList *keywords[], Accessor &styler) {
    ...
    //styler.Flush();
    // Scintilla sometimes calls the lexer with a length that ends in a multiline segment.
    // If so, we notify the start position of that Line to Scintilla here:
    ...
    for (Sci_PositionU at = startPos; at < startPos + length; at++) {
          // Use a buffer for convenient keyword matching.
        slineBuffer.resize(slineBuffer.size()+1);
        slineBuffer[linePos++] = styler[at];
        
        // End of line met, get (multi) line length.
            if (AtEOL(styler,at)) {
            Sci_PositionU lineLength=GetLineLen(styler, at);
            if (lineLength==0) lineLength++;
            
            // Multiline found. Copy the remaining chars to the lineBuffer.
    ...
    if (linePos>0){ // handle the last line (doesnt have EOL)
	 //std::clog << "last Line: [" << slineBuffer << "]\n";
...
LexerModule lmMake(SCLEX_MAKEFILE, ColouriseMakeDoc, "makefile", NULL, makefileWordListDesc);
