-- Wordle für SciTE – funktionierende Version mit eingefärbter Buchstabenübersicht
...
-- Neue Funktion: Buchstabenübersicht eingefärbt anzeigen
local function DrawLetterStatus()
  local letterStyles = {} -- z.B. A=1 (grün), B=3 (grau) etc.
  ...
  for _, guess in ipairs(GUESSES) do
    ...
    for i = 1, #styled do
      ...
      -- Immer höchsten Stil behalten: grün > gelb > grau
  ...
  for c = string.byte("A"), string.byte("Z") do
    ...
    local style = letterStyles[ch]
    if style then
      editor:StartStyling(editor.CurrentPos, 31)
      editor:SetStyling(1, style)
    else
      editor:StartStyling(editor.CurrentPos, 31)
      editor:SetStyling(1, 0)
    end
    editor:AddText(ch)
    editor:StartStyling(editor.CurrentPos, 31)
    editor:SetStyling(1, 0)
