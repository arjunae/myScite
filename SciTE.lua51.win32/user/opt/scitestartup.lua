...
-- http://www.test.de/?key1=test&key2=a12
	...
	if editor.Lexer~=1 then -- Performance: Exclude Null Lexer	
		...
		local X,eA = editor:findtext(mask_b, SCFIND_REGEXP, 0)
		while X do
			if editor:IndicatorValueAt(marker_a,X)==1 then
				EditorMarkText(X, (eA-X), marker_b)
			end -- common.lua
			X,eA = editor:findtext( mask_b, SCFIND_REGEXP, X+1)
		end
		...
		local X,eB = editor:findtext(mask_c, SCFIND_REGEXP, 0)
		while X do
			if editor:IndicatorValueAt(marker_a,X)==1 then
				EditorMarkText(X+1, (eB-X)-1, marker_c) -- common.lua
			end 
			X,eB = editor:findtext( mask_c, SCFIND_REGEXP, X+1)
		end
	end
