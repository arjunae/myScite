...
REM
if exist vc.*.*.build if not exist vc.*.%BUILDTYPE%.build (
   if "%buildtype%" neq "clean" choice /C YN /M "A different VC Build has been found. Rebuild as %BUILDTYPE%? "
   if [%ERRORLEVEL%]==[2] ( goto en ) else if [%ERRORLEVEL%]==[1] (  del /s /q *.exe *.o *.obj *pdb *.dll *.res *.map *.exp *.lib *.plist *.build 1>NUL 2>NUL & cd .. )
)
...
cd ..\..\lexilla\src
if not exist ..\bin ( Echo lexilla\bin directory not found. Creating... & mkdir ..\bin )
nmake doesnt write its errlog to stdout, need to parse the /X param
nmake /X %tmp%\nmakeErr /NOLOGO %parameter1% -f lexilla.mak | "../../uk.exe" %tmp%\scitelog.txt
findstr /n /c:"error"  %tmp%\nmakeErr
if [%errorlevel%] EQU [0] echo Stop: An Error occured while compiling Lexilla & goto en
cd
...
if [%ERRORLEVEL%]==[1] ( del /s /q *.o *.obj *pdb *.dll *.res *.map *.exp *.lib *.plist *.build & goto :start ) else (goto en )
)
...
if exist ..\bin\SciTE.exe  (copy ..\bin\SciTE.exe %ReleaseDir% >NUL ) else (echo Error: cant find SciTE.exe & goto en )
if exist ..\bin\Scintilla.dll (copy ..\bin\SciLexer.dll %ReleaseDir% >NUL ) else (echo Error: cant find Scintilla.dll & goto en) 
if exist ..\bin\Lexilla.dll (copy ..\bin\SciLexer.dll %ReleaseDir% >NUL ) else (echo Error: cant find Lexilla.dll & goto en) 
...
echo.
del /s /q *.o *.obj *pdb *.dll *.res *.map *.exp *.lib *.plist 
goto en
