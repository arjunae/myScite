...
std::string ScintillaCall::StringOfRange(Span span) {
	if (span.Length() == 0) {
		...
		std::string text(span.Length(), '\0');
		TextRangeFull tr{ {span.start, span.end}, &text[0] };
		GetTextRangeFull(&tr);
		return text;
	}
}
...
Position ScintillaCall::ReplaceTargetMinimal(bpstd::string_view text) {
